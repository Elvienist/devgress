...
public class StudentsDevicesController {
    ...
    @FXML
    public void handleStudentFilterActive() {
    ...
    @FXML
    public void handleStudentFilterInactive() {
    ...
    @FXML
    public void handleDeviceFilterActive() {
    ...
    @FXML
    public void handleDeviceFilterInactive() {
    ...
    private void loadStudentsFromDatabase() {
        ...
        String sql = "SELECT s.*, (SELECT COUNT(*) FROM devices d WHERE d.owner_id = s.student_id) as dev_count FROM students s ORDER BY s.student_code ASC";
        try (Connection conn = DBConnection.connect();
             PreparedStatement pst = conn.prepareStatement(sql);
    ...
    private void loadDevicesFromDatabase() {
        ...
        String sql = "SELECT d.*, s.full_name as owner_name FROM devices d LEFT JOIN students s ON d.owner_id = s.student_id ORDER BY d.device_id DESC";
        try (Connection conn = DBConnection.connect();
             PreparedStatement pst = conn.prepareStatement(sql);
             ResultSet rs = pst.executeQuery()) {
            while (rs.next()) {
    ...
    public void handleSaveStudent() {
        ...
        if (code.isEmpty()) {
            showInlineError(lblErrorStudentCode, "Student Code is required.");
            hasError = true;
        }
        if (name.isEmpty()) {
            showInlineError(lblErrorStudentName, "Full Name is required.");
            hasError = true;
        }
        if (course.isEmpty()) {
            showInlineError(lblErrorCourse, "Course field is required.");
            hasError = true;
        }
        if (year == null) {
            showInlineError(lblErrorYearLevel, "Please select a year level.");
            hasError = true;
        }
        ...
        try (Connection conn = DBConnection.connect()) {
            ...
            try (PreparedStatement checkPst = conn.prepareStatement(checkSql)) {
                ...
                if (currentlySelectedStudent != null) {
                    checkPst.setInt(2, currentlySelectedStudent.getStudentId());
                }
            ...
            if (currentlySelectedStudent == null) {
                String sql = "INSERT INTO students (student_code, full_name, course, year_level, contact_number) VALUES (?, ?, ?, ?, ?) RETURNING student_id";
                try (PreparedStatement pst = conn.prepareStatement(sql)) {
                    ...
                    try (ResultSet rs = pst.executeQuery()) {
                        if (rs.next()) {
                            targetStudentId = rs.getInt(1);
                        }
                    }
                }
            } else {
                targetStudentId = currentlySelectedStudent.getStudentId();
                String sql = "UPDATE students SET student_code=?, full_name=?, course=?, year_level=?, contact_number=? WHERE student_id=?";
                try (PreparedStatement pst = conn.prepareStatement(sql)) {
                    pst.setString(1, code);
                    pst.setString(2, name);
                    pst.setString(3, course);
    ...
    public void handleSaveDevice() {
        ...
        if (serial.isEmpty()) {
            showInlineError(lblErrorSerialNumber, "Serial Number is required.");
            hasError = true;
        }
        if (type == null) {
            showInlineError(lblErrorDeviceType, "Select a device type classification.");
            hasError = true;
        }
        if (brand.isEmpty()) {
            showInlineError(lblErrorBrand, "Hardware brand name is required.");
            hasError = true;
        }
        if (model.isEmpty()) {
            showInlineError(lblErrorModel, "Hardware model reference is required.");
            hasError = true;
        }
        if (owner == null) {
            showInlineError(lblErrorDeviceOwner, "Please assign an owner to this hardware device.");
            hasError = true;
        }
        ...
        try (Connection conn = DBConnection.connect()) {
            ...
            try (PreparedStatement checkPst = conn.prepareStatement(checkSql)) {
                ...
                if (currentlySelectedDevice != null) {
                    checkPst.setInt(2, currentlySelectedDevice.getDeviceId());
                }
                try (ResultSet rs = checkPst.executeQuery()) {
                    if (rs.next() && rs.getInt(1) > 0) {
            ...
            if (currentlySelectedDevice == null) {
                String sql = "INSERT INTO devices (serial_number, brand, model, device_type, owner_id, status) VALUES (?, ?, ?, ?, ?, 'ACTIVE') RETURNING device_id";
                try (PreparedStatement pst = conn.prepareStatement(sql)) {
    ...
    private void clearStudentInlineErrors() {
        Label[] studentErrors = {lblErrorStudentCode, lblErrorStudentName, lblErrorCourse, lblErrorYearLevel};
        for (Label lbl : studentErrors) {
            if (lbl != null) {
    ...
    private void clearDeviceInlineErrors() {
        Label[] deviceErrors = {lblErrorSerialNumber, lblErrorDeviceType, lblErrorBrand, lblErrorModel, lblErrorDeviceOwner};
        for (Label lbl : deviceErrors) {
            if (lbl != null) {
                lbl.setVisible(false);
                lbl.setManaged(false);
            }
    ...
    private void clearStudentFields() {
        ...
        txtCourse.clear();
        cmbYearLevel.setValue(null);
        txtContactNumber.clear();
    }

    private void clearDeviceFields() {
        txtSerialNumber.clear();
        cmbDeviceType.setValue(null);
        txtBrand.clear();
        txtModel.clear();
        cmbDeviceOwner.setValue(null);
    }
        ...
        return new TableCell<>() {
            ...
            protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    lbl.setAlignment(Pos.CENTER);
                    lbl.setPrefWidth(90);
                    String style = "-fx-padding: 6 12; -fx-background-radius: 50; -fx-font-weight: bold; -fx-font-size: 12px;";
                    if ("AMENDED".equalsIgnoreCase(item)) {
                        lbl.setStyle(style + " -fx-background-color: #FEF3C7; -fx-text-fill: #D97706;");
                    } else if ("VOIDED".equalsIgnoreCase(item)) {
                        lbl.setStyle(style + " -fx-background-color: #FEE2E2; -fx-text-fill: #B91C1C;");
                    } else {
                        lbl.setStyle(style + " -fx-background-color: #DCFCE7; -fx-text-fill: #15803D;");
                    }
                    setGraphic(new StackPane(lbl));
                }
            }
    ...
    private TableCell<ActivityLog, ActivityLog> buildDetailsCell() {
        return new TableCell<>() {
            ...
            protected void updateItem(ActivityLog item, boolean empty) {
                ...
                if (!"AMENDED".equals(status) && !"VOIDED".equals(status)) {
                    setGraphic(null);
                    setText(null);
                    return;
                }
                ...
                VBox box = new VBox(4, toggle, detailLabel);
                setGraphic(box);
                setText(null);
            }
        };
    ...
    private void handleViewStudentProfile(Student student) {
        ...
        lblProfileStudentContact.setText(student.getContactNumber() == null || student.getContactNumber().isEmpty() ? "—" : student.getContactNumber());
    ...
    @FXML
    public void handleBackFromStudentProfile() {
        paneStudentProfileView.setVisible(false);
        paneStudentsView.setVisible(true);
    }
    ...
    private void loadStudentActivityLog(int studentId) {
        ...
        String sql =
                "SELECT " +
                        ...
                        "    d.brand, " +
                        "    d.model, " +
                        "    dl_in.log_time AS time_in, " +
                        "    dl_out.log_time AS time_out, " +
                        "    dl_in.status, " +
                        ...
                        "    WHERE dl2.device_id = dl_in.device_id " +
                        "      AND dl2.direction = 'OUT' " +
                        "      AND dl2.log_time > dl_in.log_time " +
                        "    ORDER BY dl2.log_time ASC " +
                        "    LIMIT 1 " +
                        ") dl_out ON true " +
    ...
    @FXML
    public void handleBackFromDeviceProfile() {
        paneDeviceProfileView.setVisible(false);
        paneDevicesView.setVisible(true);
    }
