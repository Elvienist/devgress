...
public class StudentUpdateRequestController {
    ...
    public void initialize() {
        ...
        studentRefId = session.getStudentRefId() != null ? session.getStudentRefId() : null;
    ...
    private void loadPastRequests() {
        ...
        try (Connection conn = DBConnection.connect();
            ...
            while (rs.next()) {
                ...
                if (ts != null) {
                    ZonedDateTime zdt = ts.toInstant().atZone(ZoneId.of("Asia/Manila"));
                    formattedDate = zdt.format(DateTimeFormatter.ofPattern("MMM dd, yyyy • HH:mm"));
                }
