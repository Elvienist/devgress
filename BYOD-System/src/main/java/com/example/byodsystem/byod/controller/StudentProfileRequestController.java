...
public class StudentProfileRequestController {
    ...
    public void initializeSession(int userId, boolean isFirstLogin, String studentRefId) {
    ...
    private void openDetailRow(String label, String value) {
    ...
    private void executeApproval(String remarks) {
        ...
        try (Connection conn = DBConnection.connect()) {
            ...
            String safeRemarks = remarks.isEmpty() ? "Approved by Admin" : remarks;
            AuditLogger.log(conn, adminUserId, "REQUEST_APPROVED", "ProfileUpdateRequest", selectedRequestId,
                    "{\"request_id\":" + selectedRequestId + ",\"remarks\":\"" + safeRemarks + "\"}");
    ...
    private void executeRejection(String remarks) {
        ...
        try (Connection conn = DBConnection.connect()) {
            ...
            AuditLogger.log(conn, adminUserId, "REQUEST_REJECTED", "ProfileUpdateRequest", selectedRequestId,
                    "{\"request_id\":" + selectedRequestId + ",\"reason\":\"" + remarks + "\"}");
    ...
    private void loadRequests(String statusFilter) {
        String sql = """
                SELECT pur.request_id, s.full_name, pur.field_name,
                       ...
                       pur.status, pur.admin_response, 
                       TO_CHAR(pur.submitted_at, 'YYYY-MM-DD HH24:MI:SS') AS ts
        ...
        try (Connection conn = DBConnection.connect();
            ...
            while (rs.next()) {
                ...
                String submittedStr = rs.getString("ts");
                if (submittedStr == null) submittedStr = "—";
